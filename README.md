...
| `conversation_send` | 发消息。目标可以是**本工作区侧栏可见的任何对话**、你的成员，或你的监督者。**首次接触会在同一次调用里把对方登记成成员**（`name` 指定成员名，默认用它的 handle；返回值 `bound: true` 表示这次建立了边）。目标没打开时**自动打开再投递**（返回 `opened: true`）。`queue`（默认，开新 turn）/ `steer`（插入对方下一步）/ `inject`（只补上下文，不唤醒） |
